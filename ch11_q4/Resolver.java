...
//< resolve-local
    ...
    private void declare(Token name) {
        ...
        if (scope.locals.containsKey(name.lexeme)) {
            Lox.error(name, "Already a var with this name in scope.");
        }

        int slot = scope.nextSlot++;
        scope.locals.put(name.lexeme, new Local(name, slot));
    }
    ...
    private void resolveLocal(Expr expr, Token name) {
        for (int i = scopes.size() - 1; i >= 0; i--) {
            ...
            if (local != null) {
                ...
                interpreter.resolve(expr, depth, local.slot);
                return;
            }
        }
